...
#      print STDERR "[$authorString]($author)";
	...
	$toc{"$paperID"} = $title;
        ...
        $buf .= printPaper($volume,$title,$paperID,$href,$videoString,$mrfString,@authors);
      }
    ...
    ## Min: added the below line on Sun Jun 15 01:30:47 SGT 2014
    ## Min: BUGGY, only handles one type (latexml)
    } elsif (/<mrf src=\"([^\"]+)\">(([^\.]+).+)<\/mrf>/i) { 
      my $title = "";
      if ($1 eq "latexml") { 
	$title = "Generated by LaTeXML, for publications originating from TeX sources";
      }
      if ($title ne "") {
	$mrfString = " [<SPAN TITLE=\"Machine Readable Format\">MRF</SPAN>: <A HREF=\"$2\" TITLE=\"$title\">$1</A>]";
      } else {
	$mrfString = " [<SPAN TITLE=\"Machine Readable Format\">MRF</SPAN>: <A HREF=\"$2\">$1</A>]";
      }
...
sub printPaper {
  my ($volume, $title, $paperID, $href, $videoString, $mrfString, @authors) = @_;
